...
>(({ className, children, hideCloseButton = false, ...props }, ref) => {
  ...
  React.useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      if (!hasTitle) {
        console.warn(
          '⚠️ DialogContent にはアクセシビリティのため DialogTitle が必要です。' +
          'タイトルを隠したい場合は VisuallyHidden コンポーネントでラップしてください。'
        );
      }
      if (!hasDescription) {
        console.warn(
          '⚠️ DialogContent にはアクセシビリティのため DialogDescription が推奨されます。' +
          'または aria-describedby を設定してください。'
        );
      }
    }
  ...
  return (
    <DialogPortal>
      ...
      >
        {/* 自動的にアクセシビリティ要素を追加 */}
        {!hasTitle && process.env.NODE_ENV === 'development' && (
          <DialogTitle className="sr-only">
            ダイアログ
          </DialogTitle>
        )}
        {!hasDescription && process.env.NODE_ENV === 'development' && (
...
// アクセシビリティ修正済みDialogContentのラッパー
